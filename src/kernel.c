...
void onTCPPocket(char* pkt){
    ...
    if(strcmp(hostname,"server")==0){ // 自己是服务端 远端就是客户端
        local_ip = inet_network("10.0.0.1"); //inrt_network()函数用来把ip（ASCII码形式）转换为int形式
    ...
    //printf("my ip is %d , remote ip is %d \n" , local_ip , remote_ip);
    int hashval;
...
void* receive_thread(void* arg){
    ...
    while(1) {
        ...
        //printf("receve udp packet len:%d \n",len);
        //printf("datasize is %ld \n",sizeof(hdr));
